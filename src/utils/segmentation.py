...
class Segmentation:
    ...
    def get_beats(self, fln):
        ...
        bt_samples = [int(np.round(db[i, 0] * self.args['sr'])) for i in range(len(db)) if
                      db[i, 1] != 1 or db[i, 1] == 1]
    ...
    def process_inputs(self):
        ...
        if self.inputC is not None:
            inputC_Beats, inputC_beats, inputC_Measures, inputC_measures, inputC_onsets, inputC_onsets_samples, inputC_act = self.get_beats(
                self.flnC)
            temp_measures = [];
            inputC_measures = [0 if inputC_measures[i] > len(self.inputC) else temp_measures.append(inputC_measures[i])
                               for i in range(len(inputC_measures))];
            inputC_measures = temp_measures
            temp_beats = [];
            inputC_beats = [0 if inputC_beats[i] > len(self.inputC) else temp_beats.append(inputC_beats[i]) for i in
                            range(len(inputC_beats))];
            inputC_beats = temp_beats
            temp_measures = [];
            inputC_measures = [
                0 if inputC_measures[i] / float(self.args['hoplen']) > len(inputC_act) else temp_measures.append(
                    inputC_measures[i]) for i in range(len(inputC_measures))];
            inputC_measures = temp_measures

        inputA_Beats, inputA_beats, inputA_Measures, inputA_measures, inputA_onsets, inputA_onsets_samples, inputA_act = self.get_beats(
            self.flnA)

        temp_measures = [];
        inputA_measures = [0 if inputA_measures[i] > len(self.inputA) else temp_measures.append(inputA_measures[i]) for
                           i in range(len(inputA_measures))];
        inputA_measures = temp_measures
        temp_beats = [];
        inputA_beats = [0 if inputA_beats[i] > len(self.inputA) else temp_beats.append(inputA_beats[i]) for i in
                        range(len(inputA_beats))];
        inputA_beats = temp_beats
        temp_pattern = [];
        inputA_pattern = [0 if inputA_onsets[i] > len(self.inputA) else temp_pattern.append(inputA_onsets[i]) for i in
                          range(len(inputA_onsets))];
        inputA_pattern = temp_pattern
        temp_measures = [];
        inputA_measures = [
            0 if inputA_measures[i] / float(self.args['hoplen']) > len(inputA_act) else temp_measures.append(
                inputA_measures[i]) for i in range(len(inputA_measures))];
        inputA_measures = temp_measures

        inputB_Beats, inputB_beats, inputB_Measures, inputB_measures, inputB_onsets, inputB_onsets_samples, inputB_act = self.get_beats(
            self.flnB)

        temp_measures = [];
        inputB_measures = [0 if inputB_measures[i] > len(self.inputB) else temp_measures.append(inputB_measures[i]) for
                           i in range(len(inputB_measures))];
        inputB_measures = temp_measures
        temp_beats = [];
        inputB_beats = [0 if inputB_beats[i] > len(self.inputB) else temp_beats.append(inputB_beats[i]) for i in
                        range(len(inputB_beats))];
        inputB_beats = temp_beats
        temp_pattern = [];
        inputB_pattern = [0 if inputB_onsets[i] > len(self.inputB) else temp_pattern.append(inputB_onsets[i]) for i in
                          range(len(inputB_onsets))];
        inputB_pattern = temp_pattern
        temp_measures = [];
        inputB_measures = [
            0 if inputB_measures[i] / float(self.args['hoplen']) > len(inputB_act) else temp_measures.append(
                inputB_measures[i]) for i in range(len(inputB_measures))];
        inputB_measures = temp_measures
        ...
        temp_beats = [];
        inputB_beats = [inputB_beats[i] if inputB_beats[i] > inputB_measures[-1] else temp_beats.append(inputB_beats[i])
                        for i in range(len(inputB_beats))];
        inputB_beats = temp_beats  # clear beats after last downbeat
        temp_beats = [];
        inputB_beats = [inputB_beats[i] if inputB_beats[i] < inputB_measures[0] else temp_beats.append(inputB_beats[i])
                        for i in range(len(inputB_beats))];
        inputB_beats = temp_beats  # clear beats before first downbeat
        temp_pattern = [];
        inputB_pattern = [0 if inputB_pattern[i] > inputB_measures[-1] else temp_pattern.append(inputB_pattern[i]) for i
                          in range(len(inputB_pattern))];
        inputB_pattern = temp_pattern  # clear pat events after last downbeat
